...
const TicketItem = ({
    ...
    const setTicketTypeHandler = (e: any, ticket: any) => {
        ...
        if (existingSelectedTicketIndex >= 0) {
            const existingSelectedTicket =
                ticketTypes[existingSelectedTicketIndex];

            console.log(existingSelectedTicket);

            const otherTickets = ticketTypes.filter(
                (item: any) =>
                    item.ticket.type !== existingSelectedTicket.ticket.type
            );
    ...
    return (
        <button
            type="button"
            className={`flex w-[45%] xlg:w-full mb-[2rem] px-[2.4rem] py-[2.2rem] items-center mr-[3.2rem] last:mr-0 border rounded-[0.8rem] transition-all ease-in duration-150 hover:text-color-purple-1 hover:bg-[#F8F4FF] hover:border-color-purple-1 flex-wrap  ${
